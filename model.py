...
def softmax(evidence):    
    mu = torch.sum(torch.exp(evidence), dim=1)  # 200    
    zi = torch.exp(evidence)  # 200*10
    zi = torch.t(zi)
    ans  = torch.div(zi, mu)
    return torch.t(ans)
...
def BBA1(evidence):
    ...
    for idx in range(len(evidence)):
        data = evidence[idx]   # 200*10
        E = torch.sum(data, dim=1, keepdim=True)  # 200
        E_p = E.expand(data.shape)
        bba1[idx] = torch.mul(E_p, softmax(data))   # 200  200*10
    return bba1
    
def BBA2(evidence):    
    bba2 = dict()
    map = torch.zeros(evidence[0].shape).cuda()
    for idx in range(len(evidence)):
        data = evidence[idx]   # 200*10
        bba2[idx] = torch.max(map, data)  # 200  200*10
    return bba2    
...
class ZHAO(nn.Module):
    def __init__(self,classes, dims):
        ...
        self.Classifiers = nn.ModuleList([Classifier(dims[i],self.classes) for i in range(2)])
    ...
    def forward(self, X, y):
        loss, D_alpha, alphap = 0, dict(), dict()
        evidence = self.infer(X)
        evidence1 = BBA1(evidence)
        evidence2 = BBA2(evidence)
        
        for view_num in range(len(X)):
            alphap[0] = evidence1[view_num]+1
            alphap[1] = evidence2[view_num]+1
            D_alpha[view_num] = self.Evidence_fusion(alphap)            
